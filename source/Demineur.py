# Créé par loann, le 17/04/2023 en Python 3.7
import tkinter
...
def créer_terrainMiné(terrainMiné):
    ...
    for rangée in range(0,20):
        listeRangée = []
        for colonne in range(0,20):
...
def quand_cliqué(event):
    ...
    if gameOver == False:
        if terrainMiné[rangée][colonne] == 1:
            gameOver = True
            carré.config(bg = "red")
            messagebox.showinfo("Demineur", "GAME OVER ! Tu as touché une bombe!")
            ...
            if carrésAVérifier == 0:
                gameOver = True
                messagebox.showinfo("Demineur", "Tu as gagné, bien joué")
